...
This section presents the problem classes used in the experiments. The examples
are based on the application examples discussed by Nesterov. For each problem,
we give the original formulation, explain its interpretation, and show the
basic max-representation that connects it with the smoothing framework.

\subsection{Matrix games}

The first example is the matrix game problem
\[
    \min_{x\in\Delta_n}
    \max_{u\in\Delta_m}
    u^\top A x,
\]
where \(A\in\mathbb{R}^{m\times n}\), and \(\Delta_n\) and \(\Delta_m\) are
probability simplexes.

This problem can be interpreted as a two-player zero-sum game. The vector
\(x\) is the mixed strategy of the minimizing player, and \(u\) is the mixed
strategy of the maximizing player. The quantity \(u^\top A x\) is the expected
payoff. The minimizing player chooses \(x\) in order to make the worst possible
expected payoff as small as possible.

Since maximizing a linear function over the simplex selects the largest
coordinate, the problem can also be written as
\[
    \min_{x\in\Delta_n}
    \max_{1\le j\le m}(Ax)_j .
\]
Thus the objective is the maximum of finitely many linear functions.

\subsection{Continuous location problem}

The second example is the continuous location problem
\[
    \min_{\|x\|_2\le r}
    \sum_{j=1}^p w_j\|x-c_j\|_2.
\]
Here \(x\) is the location of a facility, \(c_j\in\mathbb{R}^d\) are fixed
locations of cities or customers, and \(w_j>0\) are weights. The feasible region
is a Euclidean ball of radius \(r\).

The objective is the total weighted distance from the facility to the given
points. The problem is convex, but it is non-smooth at points where \(x\)
coincides with one of the locations \(c_j\).

The max-structure follows from the dual representation of the Euclidean norm:
\[
    \|x-c_j\|_2
    =
    \max_{\|u_j\|_2\le 1}\langle x-c_j,u_j\rangle .
\]
Therefore,
\[
    \sum_{j=1}^p w_j\|x-c_j\|_2
    =
    \max_{\|u_j\|_2\le 1}
    \sum_{j=1}^p w_j\langle x-c_j,u_j\rangle .
\]
This puts the problem into the same general form, with auxiliary variables
\(u_1,\ldots,u_p\).

\subsection{Maximum of absolute values problem}

The third example is the maximum of absolute values problem
\[
    \min_{\|x\|_2\le r}
    \max_{1\le j\le m}|a_j^\top x-b_j|.
\]
Here \(a_j\in\mathbb{R}^n\), \(b_j\in\mathbb{R}\), and the feasible set is a
Euclidean ball.
...
\]
Hence the objective is the maximum of \(2m\) affine functions. If
\[
    \widetilde A=
    \begin{bmatrix}
        A\\
...
\[
    ...
    \max_{1\le \ell\le 2m}
    \left\{
        \widetilde a_\ell^\top x+\widetilde b_\ell
    \right\}.
\]
Equivalently, using the simplex \(\Delta_{2m}\), this can be written in
max-form as
\[
    \min_{\|x\|_2\le r}
    \max_{u\in\Delta_{2m}}
    \left\{
        \langle \widetilde A x,u\rangle+\widetilde b^\top u
    \right\}.
\]
This is a piecewise-linear convex problem of the same max-type as the matrix
game example.

\subsection{Sum of absolute values problem}

The fourth example is the sum of absolute values problem
\[
    \min_{\|x\|_2\le r}
    \sum_{j=1}^m |a_j^\top x-b_j|.
\]

Each term has the max-representation
\[
    |a_j^\top x-b_j|
    =
    \max_{|u_j|\le 1}u_j(a_j^\top x-b_j).
\]
Consequently,
\[
    \sum_{j=1}^m |a_j^\top x-b_j|
...
\]
Thus, the problem fits Nesterov's structured max-form with the auxiliary set
\(Q_2=[-1,1]^m\). This differs from the matrix game and maximum of absolute
values examples, where the auxiliary set is a simplex.
