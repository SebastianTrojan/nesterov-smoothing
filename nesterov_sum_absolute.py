    ...
    *,
    ...
    def emit_trace(
        *,
        ...
        if trace_callback is None or dual_weights is None:
            return
        dual_value = _sum_absolute_dual_value(matrix_value, offset_vector, dual_weights, radius)
        trace_callback(
            GapTracePoint(
                iteration=iteration,
                stage=stage,
    ...
    if continuation is None:
        ...
        if trace_callback is not None:
            initial_state = _sum_absolute_oracle(matrix_value, offset_vector, initial_x, mu)
            emit_trace(
                iteration=0,
                stage=1,
                mu_value=mu,
    ...
    while True:
        ...
        if trace_callback is not None and stage_offset == 0:
            initial_state = _sum_absolute_oracle(matrix_value, offset_vector, current_x, current_mu)
            emit_trace(
                iteration=0,
                stage=stage_index,
                mu_value=current_mu,
